...
def get_warp_label(flow1, flow2, label1, th=50, value=0):
    ...
    grid = np.indices((height, width)).swapaxes(0, 1).swapaxes(1, 2)  # [height, width, 2], (x, y) in location x, y
    dx = grid[:, :, 0] + flow2[:, :, 1]  # [height, width, 1]
    dy = grid[:, :, 1] + flow2[:, :, 0]  # [height, width, 1]
    sx = np.floor(dx).astype(int)
    sy = np.floor(dy).astype(int)
    valid = (sx >= 0) & (sx < height - 1) & (sy >= 0) & (sy < width - 1)  # [height, width, 1], True or False

    sx_mat = np.dstack((sx, sx + 1, sx, sx + 1)).clip(0, height - 1)  # [height, width, 4]
    sy_mat = np.dstack((sy, sy, sy + 1, sy + 1)).clip(0, width - 1)  # [height, width, 4]
    sxsy_mat = np.abs((1 - np.abs(sx_mat - dx[:, :, np.newaxis])) *
                      (1 - np.abs(sy_mat - dy[:, :, np.newaxis])))

    for i in range(4):
        flow_t = flow_t + sxsy_mat[:, :, i][:, :, np.newaxis] \
                 * flow1[sx_mat[:, :, i], sy_mat[:, :, i], :]
